...
export default function RegisterPage() {
  ...
  const [formData, setFormData] = useState({
    name: '', email: '', password: '', confirmPassword: '', phone: '', city: '', state: '',
  })
  ...
  const pwStrengthColor: Record<string, string> = { weak: 'bg-red-400', fair: 'bg-yellow-400', strong: 'bg-green-500' }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              {pwStrength && (
                <div className="mt-2 space-y-1">
                  <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all duration-300 ${pwStrengthColor[pwStrength]} ${pwStrengthWidth[pwStrength]}`} />
                  </div>
                  <p className={`text-xs font-medium ${pwStrength === 'weak' ? 'text-red-500' : pwStrength === 'fair' ? 'text-yellow-600' : 'text-green-600'}`}>
              ...
              <ul className="mt-2 space-y-0.5">
                <li className={`flex items-center gap-1.5 text-xs transition-colors ${pwLen >= 6 ? 'text-green-600' : 'text-gray-400'}`}>
                  <CheckCircle2 className={`w-3 h-3 ${pwLen >= 6 ? 'text-green-500' : 'text-gray-300'}`} />At least 6 characters
                </li>
                <li className={`flex items-center gap-1.5 text-xs transition-colors ${/[A-Z]/.test(formData.password) ? 'text-green-600' : 'text-gray-400'}`}>
                  <CheckCircle2 className={`w-3 h-3 ${/[A-Z]/.test(formData.password) ? 'text-green-500' : 'text-gray-300'}`} />One uppercase letter (recommended)
                </li>
                <li className={`flex items-center gap-1.5 text-xs transition-colors ${/\d/.test(formData.password) ? 'text-green-600' : 'text-gray-400'}`}>
                  <CheckCircle2 className={`w-3 h-3 ${/\d/.test(formData.password) ? 'text-green-500' : 'text-gray-300'}`} />One number (recommended)
                </li>
              </ul>
            </div>
            <div>
              ...
              <div className="relative">
                <input type={showConfirmPassword ? 'text' : 'password'} id="confirmPassword" name="confirmPassword"
                  value={formData.confirmPassword} onChange={handleChange} onPaste={(e) => e.preventDefault()}
                  placeholder="Re-enter your password"
                  className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-1 transition-colors pr-20 ${
                    !confirmTouched ? 'border-gray-200 focus:border-orange-500 focus:ring-orange-500'
                    : passwordsMatch ? 'border-green-400 focus:border-green-500 focus:ring-green-500'
                ...
                {confirmTouched && (
                  <span className="absolute right-4 top-1/2 -translate-y-1/2">
                    {passwordsMatch ? <CheckCircle2 className="w-4 h-4 text-green-500" /> : <XCircle className="w-4 h-4 text-red-400" />}
                  </span>
                )}
              </div>
              {confirmTouched && !passwordsMatch && <p className="text-xs text-red-500 mt-1">Passwords do not match</p>}
              {passwordsMatch && <p className="text-xs text-green-600 mt-1">Passwords match ✓</p>}
            </div>
            <div>
              <label htmlFor="phone" className="block text-sm font-medium text-gray-700 mb-1">Mobile Number <span className="text-red-500">*</span></label>
              <div className="flex">
                <span className="inline-flex items-center px-3 border border-r-0 border-gray-200 rounded-l-xl bg-gray-50 text-gray-500 text-sm select-none">+91</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" id="city" name="city" value={formData.city} onChange={handleChange} placeholder="Pune"
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-colors" />
              </div>
              <div>
            ...
            <button type="submit" disabled={loading || (confirmTouched && !passwordsMatch)}
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            ...
            <Link href="/login" className="text-orange-500 font-medium hover:underline">Login here</Link>
          </p>
        </div>
      </div>
    </div>
