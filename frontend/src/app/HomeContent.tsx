...
// This component is intentionally still a client component (it needs
// browser APIs like geolocation, URL/search-param-driven filtering, and
// interactive favorites). For SEO, the initial page-1 product + category
// data is now fetched *server-side* in page.tsx and passed in as props
// (`initialProducts`, `initialPagination`, `initialCategories`), so the
// very first HTML response already contains real listings instead of an
// empty shell that only fills in after a client-side fetch. Client-side
// fetching still runs for anything the user does after that (search,
// filters, pagination, location changes).
...
interface InitialPagination { totalPages: number; total: number; }
...
function HomeContent({
  ...
  // Only trust server-fetched initial data when the page loaded with no
  // filter/search/category query params — those cases need a client fetch
  // anyway to reflect the requested params, so we don't want stale
  // unfiltered data flashing first.
  const hasInitialData = initialProducts.length > 0 && !searchParams.toString();
  ...
  useEffect(() => {
    // Server already fetched + sorted categories for the initial render.
  ...
  useEffect(() => {
    ...
    // Server already fetched page-1, unfiltered, default-sorted products —
    // reuse that instead of firing an identical request on mount.
    if (hasInitialData && !category && !q && !city && !locality) {
      return;
    }
