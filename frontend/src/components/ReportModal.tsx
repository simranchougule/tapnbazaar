  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-gray-900 text-sm">
                {type === 'product' ? 'Report Listing' : 'Report Seller'}
              </h3>
              {targetName && <p className="text-xs text-gray-400 truncate max-w-[180px]">{targetName}</p>}
            </div>
          </div>
        ...
        <p className="text-xs text-gray-500 mb-4">Select the reason for your report. Our team reviews all reports within 24 hours.</p>

        <div className="space-y-2 mb-4">
          {reasons.map(r => (
            <label key={r} className={'flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition-colors ' +
